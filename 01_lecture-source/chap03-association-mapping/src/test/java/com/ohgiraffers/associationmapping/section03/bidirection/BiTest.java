...
public class BiTest {
    ...
    *   하지만, 객체는 서로 다른 두 단방향 참조를 합쳐서 양방향이라고 한다.
    *   따라서 두 개의 연관관계 중 주인을 설정(mappedBy)
    *   주인을 설정하는 기준은 외래키를 관리하는(가지고 있는) 엔티티가 주인이 되어야한다.
    *   [Ex] Menu 엔티티에서 categoryCode 를 가지기 때문에 주인 */
    ...
    void ownerFineTest() {
        ...
        /* comment.
        *   Menu 엔티티가 주인인 관계에서 조회 시 처음 조회부터 바로 조인 한 결과를 가져오게 된다. */
    ...
    void slaveFineTest() {
        ...
        /* comment.
         *  주인이 아닌 객체 조회 시 해당 엔티티를 먼저 조회 후
         *  필요 시 연관 된 엔티티 조회하는 쿼리를 실행하게 된다. */
    ...
    @DisplayName("양방향 연관 관계 주인 객체를 이용한 insert 테스트")
    @ParameterizedTest
    ...
    @DisplayName("양방향 연관 관계 주인이 아닌 객체를 이용한 insert 테스트")
    @ParameterizedTest

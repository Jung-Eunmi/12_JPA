...
public class LifeCycleTest {
    ...
    *   영속성 컨텍스트가 관리할 수 있게되며, 이를 영속 상태라고 한다.
    *   find(), creatQuery() 을 사용한 조회도 자동으로 영속 상태가 된다.
    *   영속 상태인 엔티티 객체는 PK 로 조회를 하면 이미 관리되고 있기때문에 같은 객체(인스턴스)를 반환해준다. */
    ...
    @ParameterizedTest
    @DisplayName("준영속화 detach 후 재영속화(marge) 테스트")
    @CsvSource({"11, 1000", "12, 1000"})
    void testDetachAndPersist(int menuCode, int menuPrice) {
        ...
        *   DB 에서 엔티티 조회 후 1차 캐시에 저장한다.
        *   조회 한 영속 엔티티 객체에 준영속 상태의 엔티티 객체의 값을 병합(merge)한 뒤
        *   영속 엔티티 객체를 반환하게 된다. */
        manager.flush();
    ...
    void testRemoveEntity(int menuCode) {
        ...
        *   트렌젝션을 플러쉬, 커밋하는 순간 영속성 컨텍스트에서 관리하는 엔티티 객체를 DB 에 반영한다. */

        manager.remove(foundMenu);
        // remove 를 반영해주라고 flush
        manager.flush();
